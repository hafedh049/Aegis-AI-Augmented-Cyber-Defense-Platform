...
def create_vms():
    ...
    for vm in vms:
        ...
        if path is None or ip is None:
            vm_path = os.path.join(BASE_VM_DIR, name, f"{name}.vmx").replace("\\", "/")
            clone_cmd = [
                "vmrun",
                "clone",
                BASE_VM_PATH,
...
def configure_ansible_master():
    ...
    ssh.exec_command("rm -rf /root/.ssh/id_rsa")
    ssh.exec_command("ssh-keygen -t rsa -b 2048 -f /root/.ssh/id_rsa -q -N ''")
